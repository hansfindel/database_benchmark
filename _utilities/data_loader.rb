class DataLoader
    ...
    def self.relational_document_factory(magnitud_order = 1)
        ...
        data.projects.each do |project_data|
            projects << { id: project_data[0], name: project_data[1], description: project_data[2], organization_id: project_data[3], created_by: project_data[4], active: project_data[5] }
        end
        project_users = []
    ...
    def self.nested_document_factory(magnitud_order = 1)
        ...
        data.users.each do |user_data|
            ...
            user_projects = []
            data.projects.each do |project_data|
                project_users << project_data[2] if project_data[1] == user[:id] # add id to user if has matches with its id
            end
            user[:project_ids] = user_projects
            users << user
        end

        projects = []
        data.projects.each do |project_data|
            project = { name: project_data[0], description: project_data[1], organization_id: project_data[2], created_by: project_data[3], active: project_data[4] }
            project_users = []
            data.project_users.each do |project_user_data|
                project_users << project_data[1] if project_data[2] == project[:id]
            end    
            project[:user_ids] = project_users
            columns = []
            data.columns.each do |column_data|
